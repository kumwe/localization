...
$workspace = rtrim(sys_get_temp_dir(), '/\\') . '/kumwe-localization-consumer-' . bin2hex(random_bytes(6));
...
consumerRun(
    [
        ...
        '--format=zip',
        '--dir=' . $distDirectory,
        '--file=kumwe-localization',
    ],
    $workspace,
);
$archive = $distDirectory . '/kumwe-localization.zip';
if (!is_file($archive)) {
...
$consumerMetadata = [
    ...
    'license' => 'proprietary',
    'require' => ['kumwe/localization' => $release, 'laminas/laminas-servicemanager' => '^4.0'],
    'repositories' => [
...
$installed = $consumer . '/vendor/kumwe/localization';
...
consumerRun([PHP_BINARY, $installed . '/examples/translate.php', $autoload], $workspace);
consumerRun([PHP_BINARY, $installed . '/examples/container.php', $autoload], $workspace);
...
foreach (array_keys($classmap) as $symbol) {
    if (
        ...
        || str_starts_with($symbol, 'Kumwe\\Localization\\Tests\\')
    ) {
