...
/**
 * Syntax-check every PHP file and hold source, workflows and prose to the 120-column limit.
 *
 ...
 * source file or workflow cannot exceed the coding standard. Machine JSON, strict single-line YAML
 * front-matter scalars and Markdown API signatures/tables are structural data, not wrappable prose.
 *
 * @since 0.1.0
 */
...
foreach ($tracked as $relative) {
    ...
    $markdown = str_ends_with($relative, '.md');
    $frontMatter = $markdown && str_starts_with($contents, "---\n");
    foreach (explode("\n", $contents) as $offset => $line) {
        ...
        if ($markdown && (str_starts_with($line, '#') || str_starts_with($line, '|'))) {
            continue;
        }
