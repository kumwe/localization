...
foreach ($symbols as $symbol => $entry) {
    ...
    if (
        !is_string($symbol)
        || !str_starts_with($symbol, 'Kumwe\\Localization\\')
        || !is_string($file)
...
if (($composer['name'] ?? null) !== 'kumwe/localization') {
    $errors[] = 'The archived composer.json does not name kumwe/localization.';
}
